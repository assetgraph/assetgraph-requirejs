...
module.exports = async function assetgraphRequireJs(assetGraph) {
  let requireJs;
  const entryPoints = [];
  ...
  if (entryPoints.length > 0) {
    ...
    try {
      for (const entryPoint of entryPoints) {
        ...
        const requireJsOptimizeOptions = {
          ...requireJsConfig,
        ...
        if (dataAlmond) {
          ...
          htmlScript.to = assetGraph.addAsset({ url: dataAlmond }, htmlScript);
        ...
        let sourceMap;
        const bundleAssetUrl = `file://${baseUrl}${
          dataMain ? `${dataMain}-` : ''
        }bundle.js`;
        if (sourceMapFileName) {
          const sourceMapContents = await readFile(sourceMapFileName, 'utf-8');
